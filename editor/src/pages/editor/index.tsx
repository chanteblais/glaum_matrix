...
const Editor = () => {
	...
	// Publish on frame updates
	useEffect(() => {
		if (!gifEnabled) {
			const data = frames.map(frame => frame.data);
			const drawing = new Drawing(data, false);
			publishingService.publish(drawing).then();
		}
	}, [publishingService, frames, gifEnabled]);
